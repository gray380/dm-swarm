...
cleanupAwsDockerswarmCluster(){
	...
	INSTANCE_STATUS=$(aws ec2 describe-instances --filters 'Name=tag:Name,Values='${NODE_PREFIX}'1'  --query  'Reservations[].Instances[].[State.Name]' --output text)
	...
	# Delete SG
	# aws ec2 delete-security-group --group-name ${CLUSTERNAME}
